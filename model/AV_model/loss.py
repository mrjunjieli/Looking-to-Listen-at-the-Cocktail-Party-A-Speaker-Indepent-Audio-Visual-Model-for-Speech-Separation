...
def audio_discriminate_loss2(gamma=0.1,beta = 2*0.1,people_num=2):
    def loss_func(S_true,S_pred,gamma=gamma,beta=beta,people_num=people_num):
        ...
        for i in range(people_num):
            sum_mtr += tf.abs(S_true[:,:,:,:,i]-S_pred[:,:,:,:,i])
            for j in range(people_num):
                if i != j:
                    sum_mtr -= gamma*(tf.abs(S_true[:,:,:,:,i]-S_pred[:,:,:,:,j]))
...
def loss (gamma=0.1,beta =2*0.1 ,people_num=2):
    def loss_func(S_true,S_pred,gamma=gamma,people_num=people_num):
        sum_mtr1 = K.zeros_like(S_true[:,:,:,:,0])


        sum_mtr = tf.abs(S_true[:,:,:,:,0]-S_pred[:,:,:,:,0])
        sum_mtr += tf.abs(S_true[:,:,:,:,1]-S_pred[:,:,:,:,1])
        sum_mtr -= gamma*(tf.abs(S_true[:,:,:,:,0]-S_pred[:,:,:,:,1]))
        sum_mtr -= gamma*(tf.abs(S_true[:,:,:,:,1]-S_pred[:,:,:,:,0]))
        loss1 = K.mean(K.flatten(sum_mtr))

        sum_mtr = tf.abs(S_true[:,:,:,:,0]-S_pred[:,:,:,:,1])
        sum_mtr += tf.abs(S_true[:,:,:,:,1]-S_pred[:,:,:,:,0])
        sum_mtr -= gamma*(tf.abs(S_true[:,:,:,:,0]-S_pred[:,:,:,:,0]))
        sum_mtr -= gamma*(tf.abs(S_true[:,:,:,:,1]-S_pred[:,:,:,:,1]))

        loss2 = K.mean(K.flatten(sum_mtr))

        if tf.less(loss1,loss2) is True:
            return loss1
        else:
            return loss2
