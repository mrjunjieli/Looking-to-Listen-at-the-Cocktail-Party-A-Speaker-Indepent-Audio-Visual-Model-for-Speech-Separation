...
class AVGenerator(keras.utils.Sequence):
    def __init__(self, pretrain,filename, database_path, X1dim=(298, 257, 2), X2dim=(75, 1, 1792, 2), ydim=(298, 257, 2, 2), batch_size=4, shuffle=True):
        'Initialization'
        self.filename = filename
        self.X1dim = X1dim
        self.X2dim = X2dim
    ...
    def __data_generation(self, filename_temp):
        ...
        for i, ID in enumerate(filename_temp):
            ...
            if(self.pretrain==True):
                X1[i, ] = np.load(self.database_path+'/mix/test/' + info[0])
                for j in range(2):
                    y[i, :, :, :, j] = np.load(self.database_path+'/crm/test/' + info[j + 1])
                    X2[i, :, :, :, j] = np.load(self.database_path+'../face1022_emb/test/' + info[j + 3])
            else:
                X1[i, ] = np.load(self.database_path+'/mix/test/' + info[0])
                for j in range(2):
                    y[i, :, :, :, j] = np.load(self.database_path+'/crm/test/' + info[j + 1])
